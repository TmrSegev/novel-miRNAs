#!/usr/bin/python
"""Generate good_candidates CSV from debugging output (generalized from Hofstenia)."""
...
def run(cfg, tool_name):
    ...
    good_candidates_df.to_csv(os.path.join(output_dir, f"{tool_name}_goodCandidates.csv"), index=False)
    filter_out_df.to_csv(os.path.join(output_dir, f"{tool_name}_filterout.csv"), index=False)
