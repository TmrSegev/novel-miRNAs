#!/usr/bin/env bash
# Sandbox regression: Hofstenia miRDeep unite + unique_candidates (pass 1 + 2).
...
rm -rf "$SANDBOX/Hofstenia/scripts" "$SANDBOX/Hofstenia/unique_candidates"
mkdir -p "$SANDBOX/Hofstenia/scripts" "$SANDBOX/Hofstenia/unique_candidates"
...
python3 mirdeepUniteGFF.py -o Hofstenia_mirdeep.gff3 --create-fasta Hofstenia_mirdeep.fasta \
  -s Hofstenia --base-path "$SANDBOX" --uniquecandidates False
...
python3 mirdeepUniteGFF.py -o Hofstenia_mirdeep.gff3 --create-fasta Hofstenia_mirdeep.fasta \
  -s Hofstenia --base-path "$SANDBOX" --uniquecandidates True
