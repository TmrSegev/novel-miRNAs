...
- Cluster path for scripts: `/groups/vaksler_group/Isana_Tzah/novel-miRNAs`
...
- **Hofstenia is the canonical shared pattern** (per-library discovery, featureCounts naming like `miRNA_miRdeep_*`). Nematodes follow it unless species-specific (path casing, nested sRNAbench out, BLAST, miRBase). sRNAbench is `sRNAbench_{lib}.sbatch` on all species; sequential nematode `srnabench.LEGACY.sbatch` is unused.
