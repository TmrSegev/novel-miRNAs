...
def run(input, output, threshold_mc, additional=None, fasta_path=None, seed_path=None):
    ...
    for index, row in table.iterrows():
        with open('/groups/vaksler_group/Isana_Tzah/RNAcentral/ncRNAs_Caenorhabditis/Caenorhabditis_rRNA.fasta') as f:
            if (row['5pseq'] in f.read()) or (row['3pseq'] in f.read()):
                row['Removal Reason'] = 'rRNA'
                deleted_input = deleted_input.append(row)
                table.drop(index=index, inplace=True)
                f.close()
        with open('/groups/vaksler_group/Isana_Tzah/RNAcentral/ncRNAs_Caenorhabditis/Caenorhabditis_snoRNA.fasta') as f:
            if (row['5pseq'] in f.read()) or (row['3pseq'] in f.read()):
                row['Removal Reason'] = 'snoRNA'
                deleted_input = deleted_input.append(row)
                table.drop(index=index, inplace=True)
                f.close()
        with open('/groups/vaksler_group/Isana_Tzah/RNAcentral/ncRNAs_Caenorhabditis/Caenorhabditis_snRNA.fasta') as f:
            if (row['5pseq'] in f.read()) or (row['3pseq'] in f.read()):
                row['Removal Reason'] = 'snRNA'
                deleted_input = deleted_input.append(row)
                table.drop(index=index, inplace=True)
                f.close()
        with open('/groups/vaksler_group/Isana_Tzah/RNAcentral/ncRNAs_Caenorhabditis/Caenorhabditis_tRNA.fasta') as f:
            if (row['5pseq'] in f.read()) or (row['3pseq'] in f.read()):
